...
def double_derivative_current(derivative_current,current,time):
    return -(decay_constant*derivative_current)-(current*(omega_not_square))+(v0*(np.sin(omega_not*time)))
...
plt.plot(time,current,'--' ,color='purple',lw=".8",label='current')
# plt.plot(t2,p,'.',color='red',label='current negative')
plt.xlabel('time')
plt.ylabel('current')
plt.title('AC LCR circuit by Tuhin Bidyanta',color='green',fontsize='20')
plt.legend()
plt.show()